...
## Configuration for one positional 2D audio stream.
##
## The playback position is supplied to [code]StdAudio.play_2d[/code] or
## [code]StdAudio.play_2d_id[/code], so one recipe can be reused anywhere.
