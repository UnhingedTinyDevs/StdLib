class_name StdAudioBook
extends Node
## A registry for storing [code]StdAudioRecipe[/code]'s by id
##
## Allows for the storing and referencing of [code]StdAudioRecipe[/code]s
## from anywhere else in the codebase. Instanced as a child of the
## StdAudio service, which exposes it through
## [code]StdAudio.register/fetch/revoke[/code].


var _recipe_book: Dictionary[StringName, StdAudioRecipe] = {}


## Add a new recipe to the book, keyed by its id. Errs when the recipe
## is null, has no id, or the id is already registered. On success the
## ok value is the recipe.
...
## Removes the desired sound from the Recipe book
